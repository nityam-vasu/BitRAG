...
import { Download, Trash2, FileText, Save, RefreshCw, Circle } from 'lucide-react';
...
export function SettingsPage() {
  ...
  const loadSettings = async () => {
    try {
      ...
      // Set Ollama status from settings API if available
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      toast.success('Settings saved successfully!', {
        description: `Model: ${data.model}, Port: ${data.ollamaPort}`,
      });
      
      // Refresh model list after saving settings
      loadModels();
    } catch (error) {
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-gray-100 dark:bg-gray-900 border border-gray-300 dark:border-gray-800 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:border-gray-400 dark:focus:border-gray-700"
                    placeholder="11434"
                  />
                  <button
                    onClick={checkOllamaConnection}
                    disabled={checkingConnection}
                    className="flex items-center gap-1 px-3 py-2 bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg transition-colors disabled:opacity-50"
                ...
                <div className="flex items-center gap-2 mt-2">
                  <Circle className={`w-2 h-2 ${
                    ollamaStatus === 'connected' ? 'text-green-500 fill-green-500' :
                    ollamaStatus === 'checking' ? 'text-yellow-500 fill-yellow-500' :
